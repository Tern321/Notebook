class BranchPageController {
    ...
    static textAreasHasFocus(): boolean {
        if (BranchPageController.argumentTextArea().matches(":focus")) { return true; }
    ...
    static async reloadUIForBranch(branchInformation: BranchData)
    {
        BranchPageController.branch = await Branch.CreateBranch(branchInformation, AccountManager.branchLink(branchInformation.id).encryptionKey);
        UIDrawer.drawUI();
    }
    ...
    static async test() {
        ...
        //ancor = ;
        if (ancor.length > 0) {
            var linkData: BranchLinkJson = JSON.parse(Base32.base16Decode(ancor));
            ...
            //console.log(branchInformation);
            //console.log("===== ");
            var text: string = await Network.getBranchData(AccountManager.userId, AccountManager.userPassword, linkData.branchId, linkData.password, branchInformation.dumpVersion, branchInformation.commandsVersion);
            var branchData = JSON.parse(text) as BranchDataJson;
