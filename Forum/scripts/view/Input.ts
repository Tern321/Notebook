...
function enableInput()
{
    ...
    document.addEventListener('keydown', function (e) {
        //console.log(e);
        if (e.keyCode == 46) {
            BranchPageActionsController.deleteContention();
        }
    }, false);
    ...
    document.getElementById("argumentTextArea").addEventListener('focus', (event) => {
        ...
        if (BranchPageController.changeSelectedContention)
        {
...
function keyUp(event) {
    if (event.keyCode == 16)
    {
        BranchPageActionsController.saveContentionOrder()
...
function checkKeycode(event)
{
    ...
    if ((event.keyCode == 0xA) || (event.keyCode == 0xD)) {
        if (event.shiftKey) {
            BranchPageActionsController.addContentionList();
        }
        else if (universalCtrlPressed(event)) {
            if (BranchPageController.changeSelectedContention) {
                BranchPageActionsController.changeContention();
            }
            else {
                BranchPageActionsController.addContentionOrUrl();
            }
        }
        else
        {
    ...
    if ([leftKeyCode, upKeyCode, rightKeyCode, downKeyCode].indexOf(event.keyCode) != -1 && universalCtrlPressed(event))
    {
        ...
        BranchPageActionsController.moveContentionSelection(event.keyCode);
        return false;
    }
    ...
    if ([upKeyCode, downKeyCode].indexOf(event.keyCode) != -1 && event.shiftKey)
    {
        ...
        BranchPageActionsController.moveContentionUp(event.keyCode == upKeyCode);
        return false;
    }
    
    if (!BranchPageController.textAreasHasFocus() && universalCtrlPressed(event))
    {
        if (event.keyCode == 67) {
            //console.log("ctrl c");
            BranchPageActionsController.copyContentionCtrlC();
            return true;
        }
        if (event.keyCode == 88) {
            //console.log("ctrl x");
            BranchPageActionsController.cutContentionCtrlX();
            return true;
        }
        if (event.keyCode == 86) {
            //console.log("ctrl v");
            BranchPageActionsController.addContentionCtrlV();
            return true;
        }
    }
...
function mouseClick(e) {
    ...
    switch (e.detail) {
        case 1:
            pendingClick = setTimeout(function () {
                ...
                if (pressTime < 800) {
                    if (e.which == 1) {
                        if (e.shiftKey) {
                            BranchPageActionsController.moveContention(contentionElement.getAttribute("id"));
                        }
                        else if (universalCtrlPressed(e)) {
                            BranchPageActionsController.moveToTopic(e, contentionElement.getAttribute("id"));
                        }
                        else {
                            BranchPageActionsController.selectContention(contentionElement);
                        }
                    }
                }
                ...
                if (e.which == 3)
                {
                    ...
                    {
                        BranchPageActionsController.addToArchive(contentionElement.getAttribute("id"));
                    }
                    else {
                        BranchPageActionsController.collapceContention(contentionElement.getAttribute("id"));
                    }
        ...
        case 2:
            ...
            if (e.which == 1) {
                BranchPageController.changeSelectedContention = true;
                BranchPageActionsController.selectContention(contentionElement);
                BranchPageActionsController.copyContentionText();
