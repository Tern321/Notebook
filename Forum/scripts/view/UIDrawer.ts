class UIDrawer {
    ...
    static addCleanObjects(parentElement: Element, contention: Contention, depth: number, x: number, y: number, drawAll: boolean)
    {
        ...
        if (!contention.collapce || BranchPageController.topicId == contention.id || drawAll) { 
        contention.childs().forEach(function (childContentionId) {
            var childContention = BranchPageController.selectedBranch().contentionForId(childContentionId);
    ...
    static drawTopics(topicContention: Contention, depth: number)
    {
        ...
        topicContention.childTopics().forEach(function (childTopicId) {
            var childTopic = BranchPageController.selectedBranch().contentionForId(childTopicId);
            UIDrawer.drawTopics(childTopic, depth + 1);
        });
    }
    static addBranchesButton() {
        var d1 = document.getElementById("topics");
        const element = document.createElement("div");
        element.innerHTML = "<button class='topicButton' style=\"background-color: #FFF; width: " + UIDrawer.topicsWidth + "px; height: 20px; \" onclick = \"Controller.openBranches()\" >Branches</button>"
        UIDrawer.topicIndex++;
        d1.appendChild(element);
    }
    ...
    static topicButtonHtml(contention: Contention, index: number, depth: number): string {
        ...
        if (contention.id == BranchPageController.topicId) {
    ...
    static drawUI() {
        var drawAll = BranchPageController.showAllEnabled;
        if (!BranchPageController.selectedBranch().contentionsMap.has(BranchPageController.topicId))
        {
        ...
        var rootKey: string = BranchPageController.topicId;
        ...
        UIDrawer.addBranchesButton();
        UIDrawer.drawTopics(BranchPageController.selectedBranch().contentionsMap.get("root"), 2);
        ...
        this.recursiveAddRawToDom(BranchPageController.selectedBranch().contentionForId(rootKey), contentionsDiv, rawElementIdList);

        // remove raw objects and save size
        rawElementIdList.forEach(function (contentionId) {
            //console.log("calcuate size for " + contentionId);
            
            var contention = BranchPageController.selectedBranch().contentionForId(contentionId);
        ...
        UIDrawer.calculateSize(BranchPageController.selectedBranch().contentionsMap.get(rootKey), 0, drawAll);

        // add clean objects
        contentionsDiv.innerHTML = "";
        this.addCleanObjects(contentionsDiv, BranchPageController.selectedBranch().contentionsMap.get(rootKey), 0, starX, startY, drawAll);

        UIDrawer.selectElement(document.getElementById(BranchPageController.selectedContentionId));

        window.scrollTo(scrollX, scrollY);

        BranchPageController.cutContentionList.forEach(function (contentionId) {
            BranchPageController.setContentionBorderType(contentionId, true);
        });
        checkWindowSize()
    }
    ...
    static selectElement(element: HTMLElement) {
        var colorTrue = BranchPageController.changeSelectedContention ? "blue" : "red";
